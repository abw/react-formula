...
class FieldContext extends BaseContext {
  ...
  async validate(callback) {
    ...
    return new Promise( await this.validator() )
      .then(
        state => {
    ...
    return async (resolve, reject) => {
      ...
      if (validate) {
        ...
        try {
          ...
          this.debug(`validate threw error for ${this.name} field:`, err)
  ...
  async validateValidator(resolve, reject) {
    const { validMessage } = { ...fieldDefaultProperties, ...this.props }
    this.debug(`calling validate function for ${this.name} field`)
    const field = this.getContext()
    try {
      const value = await this.props.validate(field.value, field, resolve, reject)
      this.debug(`validate passed for ${this.name} field:`, value)
      resolve(
        isObject(value)
          ? { message: validMessage, ...value }
  ...
  emptyValidator(resolve, reject) {
    const { requiredMessage, validMessage } = { ...fieldDefaultProperties, ...this.props }
